...
#define _DIST_MIN 175 // minimum distance to be measured (unit: mm)
...
// adjust servo position according to the USS read value
  ...
  // add your code here!
float add = (_DUTY_MAX - _DUTY_MIN)/180;
if(dist_raw < 180.0) {
     myservo.writeMicroseconds(_DUTY_MIN);//1000 0
     digitalWrite(PIN_LED,1);
  }
  else if(dist_raw < 360.0){
    myservo.writeMicroseconds(550+add*(dist_raw-180));
    digitalWrite(PIN_LED,0);
     //myservo.writeMicroseconds( _DUTY_NEU);//1500 90
  }
  else {
    myservo.writeMicroseconds(_DUTY_MAX);//2000 180
    digitalWrite(PIN_LED,1);
  }
